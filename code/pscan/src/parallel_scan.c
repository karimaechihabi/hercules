...
int main (int argc, char **argv)
{
    ...
    static unsigned int num_query_threads = 1;
    static unsigned int dbsize  = 1;            
    ...
    if(stats == NULL) {
        fprintf(stderr,"Error in parallel_scan.c: Could not allocate memory for stats structure.\n");
        return FAILURE;
    }

    init_parallel_scan_stats(stats);
    
    parallel_scan(dataset, dataset_size,queries, queries_size, timeseries_size,minimum_distance,stats,k,num_query_threads,dbsize);      
