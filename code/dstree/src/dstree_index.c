...
enum response dstree_init_stats(struct dstree_index * index)
{
    ...
    /*INDEX STATISTICS*/
    index->stats->idx_pre_traverse_tree_input_time = 0;  
    index->stats->idx_pre_traverse_tree_output_time = 0;
    index->stats->idx_pre_traverse_tree_cpu_time = 0;
    index->stats->idx_pre_traverse_tree_total_time = 0;

    index->stats->idx_pre_traverse_tree_seq_input_count = 0;
    index->stats->idx_pre_traverse_tree_seq_output_count = 0;
    index->stats->idx_pre_traverse_tree_rand_input_count = 0;
    index->stats->idx_pre_traverse_tree_rand_output_count = 0;    
    ...
    index->stats->query_total_ed_dist_count = 0;
    ...
    index->stats->query_pre_calc_ed_dist_input_time = 0;
    index->stats->query_pre_calc_ed_dist_output_time = 0;
    index->stats->query_pre_calc_ed_dist_cpu_time = 0;    
    index->stats->query_pre_calc_ed_dist_total_time = 0;    
    index->stats->query_pre_calc_ed_dist_load_node_time = 0;    

    index->stats->query_pre_calc_ed_dist_seq_input_count = 0;
    index->stats->query_pre_calc_ed_dist_seq_output_count = 0;
    index->stats->query_pre_calc_ed_dist_rand_input_count = 0;
    index->stats->query_pre_calc_ed_dist_rand_output_count = 0;    
	
    index->stats->query_calc_ed_dist_input_time = 0;
    index->stats->query_calc_ed_dist_output_time = 0;
    index->stats->query_calc_ed_dist_cpu_time = 0;    
    index->stats->query_calc_ed_dist_total_time = 0;    
    index->stats->query_calc_ed_dist_load_node_time = 0;    

    index->stats->query_calc_ed_dist_seq_input_count = 0;
    index->stats->query_calc_ed_dist_seq_output_count = 0;
    index->stats->query_calc_ed_dist_rand_input_count = 0;
    index->stats->query_calc_ed_dist_rand_output_count = 0;    

    index->stats->query_pre_calc_sax_mindist_input_time = 0;
    index->stats->query_pre_calc_sax_mindist_output_time = 0;
    index->stats->query_pre_calc_sax_mindist_cpu_time = 0;    
    index->stats->query_pre_calc_sax_mindist_total_time = 0;    
    index->stats->query_pre_calc_sax_mindist_load_node_time = 0;    

    index->stats->query_pre_calc_sax_mindist_seq_input_count = 0;
    index->stats->query_pre_calc_sax_mindist_seq_output_count = 0;
    index->stats->query_pre_calc_sax_mindist_rand_input_count = 0;
    index->stats->query_pre_calc_sax_mindist_rand_output_count = 0;    
    
    index->stats->query_calc_sax_mindist_input_time = 0;
    index->stats->query_calc_sax_mindist_output_time = 0;
    index->stats->query_calc_sax_mindist_cpu_time = 0;    
    index->stats->query_calc_sax_mindist_total_time = 0;    
    index->stats->query_calc_sax_mindist_load_node_time = 0;    

    index->stats->query_calc_sax_mindist_seq_input_count = 0;
    index->stats->query_calc_sax_mindist_seq_output_count = 0;
    index->stats->query_calc_sax_mindist_rand_input_count = 0;
    index->stats->query_calc_sax_mindist_rand_output_count = 0;

    index->stats->query_total_coaccessed_leaves_count = 0;    
    index->stats->query_total_contiguous_coaccessed_leaves_count = 0;    
 	    ...
 	    double fill_factor = ((double) index->stats->leaves_sizes[i])/index->settings->max_leaf_size;
  	    ...
  	    printf("Leaf_report_node_%d \t Fill_Factor  %f \t%s\t%d\n",
		   ...
		   fill_factor,	       
	           dataset,
	           id);
  	    printf("Leaf_report_node_%d \t Accesses  %lu \t%s\t%d\n",
		   (i+1),
		   index->stats->leaves_accesses[i],
	           dataset,
	           id);	    	    
  	    printf("Leaf_report_node_%d \t Co_Accesses  %lu \t%s\t%d\n",
		   (i+1),
		   index->stats->leaves_coaccesses[i],
	           dataset,
	           id);	    	    
	}
...
void get_query_stats(struct dstree_index * index, ts_type * ts, ts_type * paa, int query_num)
{
...
void dstree_get_query_stats(struct dstree_index * index, int query_num)
{
...
*/
/*
void dstree_print_query_stats(struct dstree_index * index, unsigned int query_num, char * queries)
{
...
*/
void print_query_stats(struct dstree_index * index, unsigned int query_num, char * queries)
{
        /*
        printf("-----------------\n");
	...
	*/
        ...
        printf("Query_pre_calc_ed_dist_input_time_secs\t%lf\t%s\t%u\n",
	       index->stats->query_pre_calc_ed_dist_input_time/1000000,	       
	       queries,
	       query_num
	     );

        printf("Query_pre_calc_ed_dist_output_time_secs\t%lf\t%s\t%u\n",
	       index->stats->query_pre_calc_ed_dist_output_time/1000000,	       
	       queries,
	       query_num
	     );

        printf("Query_pre_calc_ed_dist_load_node_time_secs\t%lf\t%s\t%u\n",
	       index->stats->query_pre_calc_ed_dist_load_node_time/1000000,	       
	       queries,
	       query_num
	     );

        printf("Query_pre_calc_ed_dist_cpu_time_secs\t%lf\t%s\t%u\n",
	       index->stats->query_pre_calc_ed_dist_cpu_time/1000000,	       
	       queries,
	       query_num
	     );
	
        printf("Query_pre_calc_ed_dist_total_time_secs\t%lf\t%s\t%u\n",
	       index->stats->query_pre_calc_ed_dist_total_time/1000000,	       
	       queries,
	       query_num
	     );

        printf("Query_pre_calc_ed_dist_seq_input_count\t%llu\t%s\t%d\n",
	       index->stats->query_pre_calc_ed_dist_seq_input_count,
	       queries,
	       query_num
	       );
	
        printf("Query_pre_calc_ed_dist_seq_output_count\t%llu\t%s\t%d\n",
	       index->stats->query_pre_calc_ed_dist_seq_output_count,
	       queries,
	       query_num
	       );
	
        printf("Query_pre_calc_ed_dist_rand_input_count\t%llu\t%s\t%d\n",
	       index->stats->query_pre_calc_ed_dist_rand_input_count,
	       queries,
	       query_num
	       );
	
        printf("Query_pre_calc_ed_dist_rand_output_count\t%llu\t%s\t%d\n",
	       index->stats->query_pre_calc_ed_dist_rand_output_count,
	       queries,
	       query_num
	       );


        printf("Query_calc_ed_dist_input_time_secs\t%lf\t%s\t%u\n",
	       index->stats->query_calc_ed_dist_input_time/1000000,	       
	       queries,
	       query_num
	     );

        printf("Query_calc_ed_dist_output_time_secs\t%lf\t%s\t%u\n",
	       index->stats->query_calc_ed_dist_output_time/1000000,	       
	       queries,
	       query_num
	     );

        printf("Query_calc_ed_dist_load_node_time_secs\t%lf\t%s\t%u\n",
	       index->stats->query_calc_ed_dist_load_node_time/1000000,	       
	       queries,
	       query_num
	     );

        printf("Query_calc_ed_dist_cpu_time_secs\t%lf\t%s\t%u\n",
	       index->stats->query_calc_ed_dist_cpu_time/1000000,	       
	       queries,
	       query_num
	     );
	
        printf("Query_calc_ed_dist_total_time_secs\t%lf\t%s\t%u\n",
	       index->stats->query_calc_ed_dist_total_time/1000000,	       
	       queries,
	       query_num
	     );

        printf("Query_calc_ed_dist_seq_input_count\t%llu\t%s\t%d\n",
	       index->stats->query_calc_ed_dist_seq_input_count,
	       queries,
	       query_num
	       );
	
        printf("Query_calc_ed_dist_seq_output_count\t%llu\t%s\t%d\n",
	       index->stats->query_calc_ed_dist_seq_output_count,
	       queries,
	       query_num
	       );
	
        printf("Query_calc_ed_dist_rand_input_count\t%llu\t%s\t%d\n",
	       index->stats->query_calc_ed_dist_rand_input_count,
	       queries,
	       query_num
	       );
	
        printf("Query_calc_ed_dist_rand_output_count\t%llu\t%s\t%d\n",
	       index->stats->query_calc_ed_dist_rand_output_count,
	       queries,
	       query_num
	       );


        printf("Query_pre_calc_sax_mindist_input_time_secs\t%lf\t%s\t%u\n",
	       index->stats->query_pre_calc_sax_mindist_input_time/1000000,	       
	       queries,
	       query_num
	     );

        printf("Query_pre_calc_sax_mindist_output_time_secs\t%lf\t%s\t%u\n",
	       index->stats->query_pre_calc_sax_mindist_output_time/1000000,	       
	       queries,
	       query_num
	     );

        printf("Query_pre_calc_sax_mindist_load_node_time_secs\t%lf\t%s\t%u\n",
	       index->stats->query_pre_calc_sax_mindist_load_node_time/1000000,	       
	       queries,
	       query_num
	     );

        printf("Query_pre_calc_sax_mindist_cpu_time_secs\t%lf\t%s\t%u\n",
	       index->stats->query_pre_calc_sax_mindist_cpu_time/1000000,	       
	       queries,
	       query_num
	     );
	
        printf("Query_pre_calc_sax_mindist_total_time_secs\t%lf\t%s\t%u\n",
	       index->stats->query_pre_calc_sax_mindist_total_time/1000000,	       
	       queries,
	       query_num
	     );

        printf("Query_pre_calc_sax_mindist_seq_input_count\t%llu\t%s\t%d\n",
	       index->stats->query_pre_calc_sax_mindist_seq_input_count,
	       queries,
	       query_num
	       );
	
        printf("Query_pre_calc_sax_mindist_seq_output_count\t%llu\t%s\t%d\n",
	       index->stats->query_pre_calc_sax_mindist_seq_output_count,
	       queries,
	       query_num
	       );
	
        printf("Query_pre_calc_sax_mindist_rand_input_count\t%llu\t%s\t%d\n",
	       index->stats->query_pre_calc_sax_mindist_rand_input_count,
	       queries,
	       query_num
	       );
	
        printf("Query_pre_calc_sax_mindist_rand_output_count\t%llu\t%s\t%d\n",
	       index->stats->query_pre_calc_sax_mindist_rand_output_count,
	       queries,
	       query_num
	       );


        printf("Query_calc_sax_mindist_input_time_secs\t%lf\t%s\t%u\n",
	       index->stats->query_calc_sax_mindist_input_time/1000000,	       
	       queries,
	       query_num
	     );

        printf("Query_calc_sax_mindist_output_time_secs\t%lf\t%s\t%u\n",
	       index->stats->query_calc_sax_mindist_output_time/1000000,	       
	       queries,
	       query_num
	     );

        printf("Query_calc_sax_mindist_load_node_time_secs\t%lf\t%s\t%u\n",
	       index->stats->query_calc_sax_mindist_load_node_time/1000000,	       
	       queries,
	       query_num
	     );

        printf("Query_calc_sax_mindist_cpu_time_secs\t%lf\t%s\t%u\n",
	       index->stats->query_calc_sax_mindist_cpu_time/1000000,	       
	       queries,
	       query_num
	     );
	
        printf("Query_calc_sax_mindist_total_time_secs\t%lf\t%s\t%u\n",
	       index->stats->query_calc_sax_mindist_total_time/1000000,	       
	       queries,
	       query_num
	     );

        printf("Query_calc_sax_mindist_seq_input_count\t%llu\t%s\t%d\n",
	       index->stats->query_calc_sax_mindist_seq_input_count,
	       queries,
	       query_num
	       );
	
        printf("Query_calc_sax_mindist_seq_output_count\t%llu\t%s\t%d\n",
	       index->stats->query_calc_sax_mindist_seq_output_count,
	       queries,
	       query_num
	       );
	
        printf("Query_calc_sax_mindist_rand_input_count\t%llu\t%s\t%d\n",
	       index->stats->query_calc_sax_mindist_rand_input_count,
	       queries,
	       query_num
	       );
	
        printf("Query_calc_sax_mindist_rand_output_count\t%llu\t%s\t%d\n",
	       index->stats->query_calc_sax_mindist_rand_output_count,
	       queries,
	       query_num
	       );
        ...
        printf("Query_total_ed_dist_count\t%llu\t%s\t%d\n",
	       index->stats->query_total_ed_dist_count,
	       queries,
	       query_num
	       );	
        ...
        printf("Query_coaccessed_leaves\t%lu\t%s\t%d\n",
	       index->stats->query_total_coaccessed_leaves_count,	       
	       queries,
	       query_num
	       );			

        printf("Query_contiguous_coaccessed_leaves\t%lu\t%s\t%d\n",
	       index->stats->query_total_contiguous_coaccessed_leaves_count,	       
	       queries,
	       query_num
	       );			
...
void get_queries_stats(struct dstree_index * index, unsigned int queries_count)
{
...
void print_queries_stats(struct dstree_index * index, int count_queries)
{
  /*
        printf("---------------------------------------- \n");    
...
/*
void dstree_get_queries_stats(struct dstree_index * index, unsigned int queries_count)
{
  ...
  unsigned int non_zero_queries_count;

  non_zero_queries_count = index->stats->total_queries_count;
  ...
  if (non_zero_queries_count != 0)
  {
    index->stats->queries_avg_eff_epsilon =  ((double) index->stats->queries_sum_eff_epsilon)
                                               / non_zero_queries_count ;
    index->stats->queries_sum_squares_eff_epsilon -= (pow(index->stats->queries_sum_eff_epsilon,2)
							/non_zero_queries_count);
    index->stats->queries_sd_eff_epsilon =  sqrt(((double) index->stats->queries_sum_squares_eff_epsilon)
						   / non_zero_queries_count);
...
void dstree_print_queries_stats(struct dstree_index * index, int count_queries)
{
        printf("Queries_refine_input_time_secs \t  %f \n", index->stats->queries_refine_input_time/1000000); 
        printf("Queries_refine_output_time_secs \t  %f \n", index->stats->queries_refine_output_time/1000000); 
        printf("Queries_refine_load_node_time_secs \t  %f \n", index->stats->queries_refine_load_node_time/1000000);
...
enum response dstree_index_write(struct dstree_index *index)
{
   ...
   //}
	...
	dstree_node_write(index, index->first_node, file, true);
        COUNT_PARTIAL_OUTPUT_TIME_START
...
enum response dstree_index_restructure_verify(struct dstree_node * node)
{

 	if(!node->is_leaf) {                
		dstree_index_restructure_verify(node->left_child);
		dstree_index_restructure_verify(node->right_child);		
	}
 	else { 
             
        struct dstree_node * parent =  node->parent;                
		if (node->accesses > 0)
		{
           if (parent->left_child->is_leaf && parent->right_child->is_leaf)
           {

 			if ( node->coaccesses/(1.0 * node->accesses) > 0.75 )
			{               
			   parent->merge = 1;
			   parent->file_pos = parent->left_child->file_pos;
			}
          }
		}
	}
...
enum response dstree_index_count_contiguous_coaccesses(struct dstree_index * index, struct dstree_node * node)
{

 	if(!node->is_leaf) {                
		dstree_index_count_contiguous_coaccesses(index, node->left_child);
		dstree_index_count_contiguous_coaccesses(index, node->right_child);		
	}
 	else { 
		...
		if (node->accesses > 0)
		{
           if (parent->left_child->is_leaf && parent->right_child->is_leaf)
           {
	           if (parent->left_child->coaccesses > 0 )
	           {
	              
                    //if (node->is_left)
                    //{     
			  	       printf("Left_node_access_order\t%lu\t\tRight_node_access_order\t%lu\t\tLeft_Node_Filename\t%s\t\n",
					         parent->left_child->access_order,
					         parent->right_child->access_order,
			           	     node->filename);
                    //}

	 			if ( abs((int)(parent->left_child->access_order - parent->right_child->access_order)) == 1 )
				{               
					++(parent->left_child->contiguous_coaccesses);
					++(parent->right_child->contiguous_coaccesses);		    		    
					COUNT_CONTIGUOUS_COACCESSED_LEAVES	
				}
              }
                    parent->left_child->coaccesses = 0;
                    parent->right_child->coaccesses = 0;
                    parent->left_child->access_order = 0;
                    parent->right_child->access_order = 0;
          }
		}
	}

    //contiguous_coaccessed_leaves_count = contiguous_coaccessed_leaves_count/2 ;
    index->stats->query_total_contiguous_coaccessed_leaves_count = contiguous_coaccessed_leaves_count;    

    return SUCCESS;
 
}

enum response dstree_index_restructure_finalize(struct dstree_index *index, struct dstree_node * node)
{
 	...
 	if(!node->is_leaf) 
        {                
           if (node->merge)
            {
               merge_node(index, node);
               node->merged = 1;
               node->merge = 0;
            }
            else 
            { 
		dstree_index_restructure_finalize(index, node->left_child);
		dstree_index_restructure_finalize(index, node->right_child);
            }         
	}
...
struct dstree_index * dstree_index_read(const char* root_directory)
{
	...
	index->stats->leaf_nodes_count = count_leaves;	
	index->stats->leaves_heights = calloc(count_leaves, sizeof(int));
	index->stats->leaves_coaccesses = calloc(count_leaves, sizeof(unsigned long));	
	index->stats->leaves_accesses = calloc(count_leaves, sizeof(unsigned long));	
	...
	index->leaves = calloc (count_leaves, sizeof(struct dstree_node *));
	  
	index->first_node = dstree_node_read(index, file, true);
        COUNT_PARTIAL_INPUT_TIME_START	
	fclose(file);
        COUNT_PARTIAL_INPUT_TIME_END
...
enum response dstree_node_write(struct dstree_index *index, struct dstree_node *node, FILE *file, boolean is_root)
{
        COUNT_PARTIAL_SEQ_OUTPUT
        COUNT_PARTIAL_SEQ_OUTPUT	
        COUNT_PARTIAL_SEQ_OUTPUT  
	...
	fwrite(&(node->accesses), sizeof(unsigned long), 1, file);       			
	fwrite(&(node->coaccesses), sizeof(unsigned long), 1, file);
	//if (!is_root)
	//  fwrite(node->parent, sizeof(struct dstree_node), 1, file);		
        //if (!is_root) fwrite(node->parent, sizeof(struct dstree_node), 1, file);		
        COUNT_PARTIAL_OUTPUT_TIME_END
	...
	if(node->is_leaf) {
		if(node->filename != NULL) {
           ...
           //if (index->settings->is_new)
  			   flush_leaf_to_leaves_file(index,node);
	...
	if(!node->is_leaf) {
		dstree_node_write(index, node->left_child, file, false);
		dstree_node_write(index, node->right_child, file, false);		
	}
	
	return SUCCESS;
}

struct dstree_node * dstree_node_read(struct dstree_index *index, FILE *file, boolean is_root) {
	...
	fread(&(node->accesses), sizeof(unsigned long), 1, file);
	fread(&(node->coaccesses), sizeof(unsigned long), 1, file);
        //fread(node->parent, sizeof(struct dstree_node), 1, file);
 	...
 	if(is_leaf) {
		...
		if(filename_size > 0)
		{
			...
			COUNT_LEAF_NODE
   	                ...
   	                index->stats->leaves_coaccesses[index->stats->leaves_counter] = node->coaccesses;			
   	                index->stats->leaves_accesses[index->stats->leaves_counter] = node->accesses;			
	...
	if(!is_leaf) {
		node->left_child = dstree_node_read(index, file, false);
		node->right_child = dstree_node_read(index, file, false);
