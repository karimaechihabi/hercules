...
enum response flush_leaf_to_leaves_file(struct dstree_index *index, struct dstree_node *node)
{
...
ts_type ** get_all_time_series_in_adaptive_node_leavesfile(struct dstree_index * index, struct dstree_node * node, int skip_index) 
{

  int ts_length = index->settings->timeseries_size;
  ts_type ** ret = NULL;
  
  unsigned long long file_position = 0;
  
  if (node->is_leaf)
     file_position = node->file_pos;
  else
     file_position = node->left_child->file_pos;
   
  //unsigned int node_size = node->left_child->node_size + node->right_child->node_size;
  
  ret = calloc(node->node_size, sizeof(ts_type *));
     
  if (!skip_index)
  {
       COUNT_PARTIAL_RAND_INPUT
       COUNT_PARTIAL_INPUT_TIME_START       
       fseek(index->leaves_raw_file, file_position * index->settings->timeseries_size * sizeof(ts_type),
	    SEEK_SET);
       COUNT_PARTIAL_INPUT_TIME_END              
  }   
     ...
     {       
       ...
       fread(ret[i],
	     sizeof(ts_type),ts_length, index->leaves_raw_file);
       COUNT_PARTIAL_INPUT_TIME_END       
     }
...
ts_type ** get_all_time_series_in_adaptive_node(struct dstree_index * index, struct dstree_node * node) 
{
...
ts_type ** get_all_time_series_in_adaptive_node(struct dstree_index * index, struct dstree_node * node) 
{
  ...
  ret = calloc(node_size, sizeof(ts_type *));
