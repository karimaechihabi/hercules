...
int main(int argc, char **argv)
{
	...
	while (1)
	{
		static struct option long_options[] = {
			...
			{"num-threads", required_argument, 0, 'B'},
		...
		case 'B':
			q_settings.num_threads = atoi(optarg);
			break;
		...
		case '?':
			printf("Usage:\n\
                       ...
                       \t--mode: 0=index, 1=query, 2=index & query\n\
	...
	if (mode == 0) // only build and store the index
	{
		...
		if (q_settings.num_threads == 1)
		{
			...
			index->stats->idx_building_total_time += partial_time + index->stats->idx_traverse_tree_total_time + index->stats->idx_append_ts_to_leaf_total_time + index->stats->idx_evaluate_split_policies_total_time + index->stats->idx_split_node_total_time;
			index->stats->idx_building_input_time += partial_input_time + index->stats->idx_traverse_tree_input_time + index->stats->idx_append_ts_to_leaf_input_time + index->stats->idx_evaluate_split_policies_input_time + index->stats->idx_split_node_input_time;
			index->stats->idx_building_output_time += partial_output_time + index->stats->idx_traverse_tree_output_time + index->stats->idx_append_ts_to_leaf_output_time + index->stats->idx_evaluate_split_policies_output_time + index->stats->idx_split_node_output_time;
			index->stats->idx_building_cpu_time += index->stats->idx_building_total_time - index->stats->idx_building_input_time - index->stats->idx_building_output_time;
			index->stats->idx_building_seq_input_count += partial_seq_input_count + index->stats->idx_traverse_tree_seq_input_count + index->stats->idx_append_ts_to_leaf_seq_input_count + index->stats->idx_evaluate_split_policies_seq_input_count + index->stats->idx_split_node_seq_input_count;
			index->stats->idx_building_seq_output_count += partial_seq_output_count + index->stats->idx_traverse_tree_seq_output_count + index->stats->idx_append_ts_to_leaf_seq_output_count + index->stats->idx_evaluate_split_policies_seq_output_count + index->stats->idx_split_node_seq_output_count;
			index->stats->idx_building_rand_input_count += partial_rand_input_count + index->stats->idx_traverse_tree_rand_input_count + index->stats->idx_append_ts_to_leaf_rand_input_count + index->stats->idx_evaluate_split_policies_rand_input_count + index->stats->idx_split_node_rand_input_count;

			index->stats->idx_building_rand_output_count += partial_rand_output_count + index->stats->idx_traverse_tree_rand_output_count + index->stats->idx_append_ts_to_leaf_rand_output_count + index->stats->idx_evaluate_split_policies_rand_output_count + index->stats->idx_split_node_rand_output_count;
		}
		else
		{

			if (!hercules_index_build(dataset, dataset_size, index, q_settings.num_threads, initial_db_size))
			{
				fprintf(stderr, "Error main.c:  Could not build the index.\n");
				return -1;
			}
		...
		index->stats->idx_writing_cpu_time = partial_time - partial_input_time - partial_output_time;
	...
	else if (mode == 1) // read an existing index and execute queries
	{
		...
		index->stats->idx_reading_cpu_time = partial_time - partial_input_time - partial_output_time;
