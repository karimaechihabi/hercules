//
//  hercules_file_buffer_manager.c
...
enum response init_file_buffer_manager(struct hercules_index *index)
{
  ...
  index->buffer_manager = malloc(sizeof(struct hercules_file_buffer_manager));
  if(index->buffer_manager == NULL) {
          fprintf(stderr,"Error in hercules_file_buffer_manager.c: Could not allocate memory for buffer manager.\n");
          return FAILURE;	
  }
  ...
  pthread_mutex_init(&(index->buffer_manager->lock_file_map), NULL);
  pthread_mutex_init(&(index->buffer_manager->lock_file_buffer), NULL);  
...
enum response set_buffered_memory_size(struct hercules_index * index)
{
  if(index == NULL)
  {
          fprintf(stderr,"Error in hercules_file_buffer_manager.c: Cannot set the buffered memory \
                           for a NULL index.\n");
          return FAILURE;	    
  }
  else
  {
        unsigned long long num_bytes = index->settings->buffered_memory_size * 1024 * 1024;
	...
	*/
	...
	unsigned long ts_size= sizeof(ts_type) * index->settings->timeseries_size;
        ...
        unsigned long size_leaf_buffer = sizeof(struct hercules_file_buffer) +
	                                       (sizeof(ts_type*) * max_leaf_size);
	
	unsigned long long mem_array_size =  (long long)((num_bytes -
        ...
        {
          fprintf(stderr,"Error in hercules_file_buffer_manager.c:"
...
enum response get_file_buffer(struct hercules_index *index, struct hercules_node *node)
{
  ...
  {
    if (!hercules_file_buffer_init(node))
    {
          fprintf(stderr,"Error in hercules_file_buffer_manager.c: Could not initialize the file buffer \
                           for the node.\n");
          return FAILURE;	    
    }
    ...
    {
          fprintf(stderr,"Error in hercules_file_buffer_manager.c: Could not add the file buffer \
                           to the map.\n");
          return FAILURE;	    
    }  
  }
...
//  the full contents of a leaf size at once.
  ...
  if (index->buffer_manager->current_record_index > buffer_limit)  
  {
      ...
      printf ("%s, batch remove in get_file_buffer! %lu  \n", curr_time, index->buffer_manager->current_record_index);
      unsigned long to_size = 0;
      free(curr_time);
       
      struct hercules_file_map * currP = NULL;
	...
	{
          fprintf(stderr,"Error in hercules_file_buffer_manager.c: Could not flush the buffer \
                          for node %s to disk.\n",currP->file_buffer->node->filename);
          return FAILURE;	
        }
        currP = currP->next;
...
/*
enum response get_file_buffer_parallel(struct hercules_index *index, struct hercules_node *node)
{
...
//  the full contents of a leaf size at once.
  ...
  if (index->buffer_manager->current_record_index > buffer_limit)  
  {
      ...
      curr_time = malloc (sizeof(char) *26);
      get_current_time(curr_time);
      printf ("%s, batch remove in buffer_manager! %lu  \n", curr_time, index->buffer_manager->current_record_index);
      unsigned long to_size = 0;
      free(curr_time);
...
enum response add_file_buffer_to_map(struct hercules_index * index, struct hercules_node *node)
{
  //    fprintf(stderr,"Adding file_buffer of node %s to map.\n", node->file_buffer->node->filename);
    ...
    if (idx == 0)
    {
      index->buffer_manager->file_map = malloc(sizeof(struct hercules_file_map));
      if(index->buffer_manager->file_map == NULL) {
        fprintf(stderr,"Error in hercules_file_buffer_manager.c: Could not allocate memory for the file map.\n");
	return FAILURE;	
      }
    ...
    {
      struct hercules_file_map * currP = index->buffer_manager->file_map;
      struct hercules_file_map * lastP = index->buffer_manager->file_map_tail;
	...
	currP = currP->next;
      ...
      */
      ...
      lastP->next = malloc(sizeof(struct hercules_file_map));

      if(lastP->next == NULL) {
        fprintf(stderr,"Error in hercules_file_buffer_manager: Could not allocate memory for new entry in the file map.\n");
	return FAILURE;	
      }
...
enum response save_all_buffers_to_disk(struct hercules_index *index)
{
  ...
  while(currP != NULL)
  {
    ...
    {
          fprintf(stderr,"Error in hercules_file_buffer_manager.c: Saving all to disk..Could not flush the buffer \
                          for node %s to disk.\n", currP->file_buffer->node->filename);
          return FAILURE;	
    }
